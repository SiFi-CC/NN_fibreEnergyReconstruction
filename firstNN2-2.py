...
with np.load(path) as data:
    ...
    model = keras.Sequential([keras.layers.InputLayer(input_shape = (12,2,32,2)),
                              ...
                              keras.layers.Dense(12000, activation = "tanh"),
                              keras.layers.Dense(8000, activation = "tanh"),
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'test'], loc='upper left')
    plt.savefig('acc_hist2-2.png')
                              


    # save model
    model.save('firstNN_model2-2.h5')
